...
class ScanNetDataset(utils.data.Dataset):
    ...
    def _get_abs_pose(self, scene_name, name0, name1):
        pose0 = self._read_abs_pose(scene_name, name0)
        pose1 = self._read_abs_pose(scene_name, name1)
    ...
    def __getitem__(self, idx):
        ...
        image0, shape0 = read_scannet_gray(img_name0, resize=(640, 480), augment_fn=None)
                                #    augment_fn=np.random.choice([self.augment_fn, None], p=[0.5, 0.5]))
        image1, shape1 = read_scannet_gray(img_name1, resize=(640, 480), augment_fn=None)
                                #    augment_fn=np.random.choice([self.augment_fn, None], p=[0.5, 0.5]))
        ...
        if self.mode in ['train', 'val']:
            ...
            #depth0 = depth1 = torch.tensor([]) :TODO check why authors implemented it like this
            depth0 = read_scannet_depth(osp.join(self.root_dir, scene_name, 'depth', f'{stem_name_0}.png'))
            depth1 = read_scannet_depth(osp.join(self.root_dir, scene_name, 'depth', f'{stem_name_1}.png'))

        # read the intrinsic of depthmap
        K_0 = K_1 = torch.tensor(self.intrinsics[scene_name].copy(), dtype=torch.float).reshape(3, 3)
        scale_factor_0 = (self.coarse_scale, self.coarse_scale)
        scale_factor_1 = (self.coarse_scale, self.coarse_scale)
        ...
        T0, T1 = self._get_abs_pose(scene_name, stem_name_0, stem_name_1)
        ...
        data = {
            ...
            'scene_id': scene_name,
            'pair_id': torch.tensor(idx),
            'pair_idxs': (torch.tensor(stem_name_0, dtype=torch.int32), torch.tensor(stem_name_1, dtype=torch.int32)),
