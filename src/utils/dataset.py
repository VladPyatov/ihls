...
def read_replica_depth(path, pad_to=None):
    if str(path).startswith('s3://'):
        depth = load_array_from_s3(path, MEGADEPTH_CLIENT, None, use_h5py=True)
    else:
        depth = cv2.imread(path, cv2.IMREAD_ANYDEPTH).astype(np.float32) / 6556.5
    if pad_to is not None:
        depth, _ = pad_bottom_right(depth, pad_to, ret_mask=False)
    depth = torch.from_numpy(depth).float()  # (h, w)
    return depth
...
def scale_intrinsics(camera_matrix: torch.Tensor, scale_factor: Tuple) -> torch.Tensor:
    """ Scale an intrinsic camera matrix.
    """
    K_scale = camera_matrix.clone()
    K_scale[..., 0, 0] *= scale_factor[0]
    K_scale[..., 0, 2] *= scale_factor[0]
    K_scale[..., 1, 1] *= scale_factor[1]
    K_scale[..., 1, 2] *= scale_factor[1]
    return K_scale
