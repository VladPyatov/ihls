...
def log_decorator(func):
    def new_func(__message, *args, **kwargs):
        func(f'RANK: {rank_zero_only.rank} | ' + __message, args, kwargs)
    return new_func


def get_rank_logger(logger: _Logger):
    for _level in logger._core.levels.keys():
        level = _level.lower()
        setattr(logger, level, log_decorator(getattr(logger, level)))
    return logger
