...
def custom_collate_fn(data: list):
    kpts_keys = ['mkpts0_f', 'mkpts1_f', 'std', 'mcosine', 'mdist', 'mconf']
    data_keys = data[0].keys()
    tensor_keys = set(data_keys) - set(kpts_keys)
    result = {}
    for key in tensor_keys:
        result[key] = default_collate([d[key] for d in data])
    return result
...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def _build_concat_dataset(
        ...
        dd  = ConcatDataset(datasets)
        print('LEN OF THE DATASET:', len(dd))
        return dd
    ...
    def train_dataloader(self):
        ...
        if self.data_sampler == 'scene_balance' and str(self.trainval_data_source).lower() in ['megadepth', 'scannet']:
